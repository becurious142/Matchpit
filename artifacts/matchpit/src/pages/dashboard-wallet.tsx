        ...
        <CardContent className="p-8">
          ...
          <div className="flex items-center justify-between p-4 bg-background/40 rounded-xl border border-border/50">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Automatically use balance when you pay for a booking</p>
            </div>
            <Switch
        ...
        <Card className="bg-card/50 border-border/50 mb-6">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-base font-bold uppercase tracking-wider">
              <Gift className="w-4 h-4 text-primary" /> Rewards Earned
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6 pt-0 space-y-3">
            {wallet.rewards.map((r) => (
              <div key={r.id} className="flex items-center justify-between py-2 border-b border-border/30 last:border-0">
                ...
                <Badge className="bg-green-500/20 text-green-400 border-green-500/30">+₹{r.amount}</Badge>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <CardContent className="p-6 pt-0">
          {wallet.ledger.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground mt-1">Book a turf or join a match to earn wallet credits.</p>
            </div>
          ) : (
            <div className="space-y-1">
              {wallet.ledger.map((entry) => (
                <div key={entry.id} className="flex items-center gap-4 py-3 border-b border-border/30 last:border-0">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                    entry.type === "credit" ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"
                  }`}>
                  ...
                  <span className={`font-bold text-sm shrink-0 ${entry.type === "credit" ? "text-green-400" : "text-red-400"}`}>
